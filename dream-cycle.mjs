...
const MESH_API_URL = process.env.MESH_API_URL || "http://127.0.0.1:18805/mesh/shared-pool";
const MESH_TIMEOUT_MS = Number(process.env.MESH_TIMEOUT_MS) || 3000;
...
async function fetchMeshFacts() {
  // 3-second timeout (FR-2.3 / AC-5): don't let a hung daemon stall the cron.
  // The URL is configurable via MESH_API_URL so tests can point at a non-routable
  // address to force the no-entries path.
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), MESH_TIMEOUT_MS);
  try {
    const res = await fetch(MESH_API_URL, { signal: controller.signal });
    if (!res.ok) throw new Error(`mesh API ${res.status}`);
    const { facts } = await res.json();
    if (!facts?.length) return [];
    return facts.map(f=>`### Mesh [${f.id}] ${f.agent_id}\n\n${f.content}`);
  } catch(e){
    // FR-2.2: every failure path returns [] with a single warn line. Never throws.
    console.warn("[dream] mesh: "+e.message);
    return [];
  } finally {
...
async function main() {
  ...
  if (totalEntries === 0) {
    // FR-3.1 / AC-4: write a no-entries file (exit 0) so the cron payload's step 3
    // can find a file to read or skip. The previous behavior (log + return) left the
    // cron payload guessing whether the cycle actually ran.
    console.log("[dream] No recent entries — writing no-entries marker");
    const noEntriesPath = resolve(MEMORY_BASE, `dream-cycle-${today}.md`);
    await mkdir(MEMORY_BASE, { recursive: true });
    await writeFile(
      noEntriesPath,
      `# Dream Cycle — ${today}\n\n_Generated by mesh-memory dream-cycle for agent: ${config.agentId}_\n\n# Dream Cycle — No Entries\n\nNo recent mesh or LCM entries found for consolidation.\n`,
      "utf-8"
    );
    console.log(`[dream] No-entries marker written to ${noEntriesPath}`);
    return;
  }
