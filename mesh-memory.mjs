...
function initDatabase() {
  ...
  // Migration: add last_seen column to pre-v2.1 shared_pool
  try {
    db.exec("ALTER TABLE shared_pool ADD COLUMN last_seen TEXT");
    db.prepare("UPDATE shared_pool SET last_seen = datetime('now')").run();
    log('INFO', 'Migrated shared_pool: added last_seen column');
  } catch(e) {
    // Column already exists (table was created with it)
  }
  ...
  `);
  // idx_sp_last_seen requires the column to exist
  try {
    db.exec('CREATE INDEX IF NOT EXISTS idx_sp_last_seen ON shared_pool(last_seen)');
  } catch(e) {
    // Column doesn't exist yet — should not happen after migration
  }
...
function addSharedFact(fact) {
  ...
  const stmt = db.prepare(`
    INSERT OR REPLACE INTO shared_pool (id, agent_id, fact_type, content, tags, timestamp, received_at, last_seen, provenance, source_agent, decay_score)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1.0)
  `);
  ...
  // Reset decay_score on re-insert (fact was seen again)
  db.prepare('UPDATE shared_pool SET decay_score = 1.0 WHERE id = ?').run(id);

  return id;
}

function getSharedFacts(opts = {}) {
  const { since, limit = 100, agentId, factType, includeExpired = false } = opts;
  let sql = 'SELECT * FROM shared_pool WHERE 1=1';
  const params = [];

  if (!includeExpired) {
    const cutoff = new Date(Date.now() - CONFIG.factTtlHours * 3600 * 1000)
      .toISOString();
    sql += ' AND timestamp >= ?';
    params.push(cutoff);
    sql += ' AND decay_score >= ?';
    params.push(CONFIG.minDecayScore);
  }
...
function pruneExpiredFacts() {
  ...
  const result = db.prepare(
    'DELETE FROM shared_pool WHERE timestamp < ?'
  ).run(cutoff);
  if (result.changes > 0) {
    log('INFO', `Pruned ${result.changes} expired fact(s) (older than ${CONFIG.factTtlHours}h)`);
  }
  return result.changes;
}

function applyDecay() {
  // Decay scores for facts not seen in the last 24h
  const result = db.prepare(`
    UPDATE shared_pool
    SET decay_score = MAX(decay_score - ?, last_seen = datetime('now'))
    WHERE last_seen < datetime('now', '-24 hours')
      AND decay_score > ?
  `).run(CONFIG.decayRatePerDay, CONFIG.minDecayScore);
  if (result.changes > 0) {
    // Also prune facts that decayed below the threshold
    const pruned = db.prepare(
      'DELETE FROM shared_pool WHERE decay_score < ?'
    ).run(CONFIG.minDecayScore);
    if (pruned.changes > 0) {
      log('INFO', `Decay pruned ${pruned.changes} fact(s) below threshold`);
    }
    log('INFO', `Decay applied to ${result.changes} fact(s)`);
  }
  return { decayed: result.changes, pruned: 0 };
}

// Run TTL/decay on every sync cycle (every 30s), skip if no facts exist
let decayCounter = 0;
const DECAY_INTERVAL = 300; // Apply decay every 5 min (10 sync cycles)

function maybeApplyDecay() {
  const count = db.prepare('SELECT COUNT(*) as n FROM shared_pool').get().n;
  if (count === 0) return;
  decayCounter++;
  if (decayCounter >= DECAY_INTERVAL) {
    decayCounter = 0;
    pruneExpiredFacts();
    applyDecay();
  }
...
// Periodic TTL/decay (independent of sync)
setInterval(() => {
  maybeApplyDecay();
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/mesh/shared-pool' && method === 'GET') {
      ...
      const facts = getSharedFacts({ since, limit, agentId, factType, includeExpired });

      // Refresh last_seen on read (prevents decay during active use)
      if (!includeExpired) {
        const ids = facts.map(f => f.id);
        if (ids.length > 0) {
          const placeholders = ids.map(() => '?').join(',');
          db.prepare(`UPDATE shared_pool SET last_seen = datetime('now') WHERE id IN (${placeholders})`).run(...ids);
        }
      }
...
function start() {
  ...
  server.listen(CONFIG.port, CONFIG.host, () => {
    ...
    // Prune any expired facts on startup
    pruneExpiredFacts();
    applyDecay();
    startSyncLoop();
    startWatcher();
